...
def ascii_generator(image_path, start_frame, number_of_frames):
    ...
    while current_frame <= number_of_frames:
        path_to_image = image_path + '/girls_band_cry_' + str(current_frame) + '.jpg'
        image = Image.open(path_to_image)
        ascii_characters = pixels_to_ascii(greyscale(resize_image(image)))  # get ascii characters
        pixel_count = len(ascii_characters)
        ascii_image = "\n".join(
            [ascii_characters[index:(index + frame_size)] for index in range(0, pixel_count, frame_size)])
        file_name = r"TextFiles/" + "girls_band_cry" + str(current_frame) + ".txt"
        try:
...
def main():
    while True:
        sys.stdout.write('==============================================================\n')
        sys.stdout.write('seleccione una opcion: \n')
        sys.stdout.write('1) reproducir\n')
        sys.stdout.write('2) salir\n')
        sys.stdout.write('==============================================================\n')

        user_input = str(input("su eleccion: "))
        user_input.strip()  # removes trailing whitespaces

        if user_input == '1':
            user_input = str(input("escriba la direccion del archivo (debe estar en el dioco local C:): "))
